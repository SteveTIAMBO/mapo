...
- **MIAPO+ standalone** ✅ déployé + vérifié live 2026-06-27 (hôte d'alors :
  `miapo.app-edufrem.com`, **retiré le 2026-08-03** au profit de
  `mapoplus.app-edufrem.com` — seul hôte MAPO+ officiel),
...
>   - ✅ **Objectif par matière + origine des révisions livrés (2026-08-03, commit `b6c7c71`)** — vérifiés en prod. `objectifs: {matiere: note}` surcharge `objectifNote` global ; **tout le seuil « en dessous de quoi on révise » passe par `objectifDe(e, matiere)`** (appelé par `faiblesses()` + digest) → la notation multi-régime ci-dessous n'aura QUE cette fonction à faire évoluer, pas les écrans. Et `addRevisionCiblee(…, origine)` distingue la demande du PARENT de la faiblesse lue sur une copie (« Demandé par ton parent » côté enfant). 23 assertions au vert.
>   - ⚠️ **Correction du backlog** : « Mes enfants » n'est PAS une sous-section des réglages — c'est déjà un module de navigation (liste + ajout + notes). Ce qui est éclaté, c'est la carte **compte enfant / lien magique** (`MiapoEnfantCompte`), rangée dans Réglages → « Mes enfants », loin du point d'ajout d'un enfant. Geste restant = la remonter dans le module (composant autonome, sans props) et laisser le co-parent en réglage.
>   - Reste du modèle famille : ce déplacement, et la migration des profils locaux. « Passer le téléphone » : déjà en place (`MiapoProfilSwitch`).
