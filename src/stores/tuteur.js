...
export const useTuteurStore = defineStore('tuteur', () => {
   ...
   */
  async function orientation6c({ niveau, pays = '', competences = {}, forts = [], faibles = [], interets = [], candidats = [], langue = 'fr' }) {
    try {
      const user = fbAuth.currentUser
      const token = user ? await user.getIdToken().catch(() => null) : null
      const headers = { 'Content-Type': 'application/json' }
      ...
      const res = await fetch(IA_URL, {
        method: 'POST',
        headers,
        body: JSON.stringify({ metered: mtrB2C(), task: 'orientation6c', data: { niveau, pays, competences, forts, faibles, interets, candidats, langue } }),
      })
      const json = await res.json().catch(() => null)
      noteCredits(json)
      if (json && json.ok && json.text) {
   ...
   */
  async function chatTuteur({ message, niveau = '', matieres = '', cours = '', historique = '', internet = false, prenom = '', langue = 'fr' }) {
    try {
      const user = fbAuth.currentUser
      const token = user ? await user.getIdToken().catch(() => null) : null
      const headers = { 'Content-Type': 'application/json' }
      ...
      const res = await fetch(IA_URL, {
        method: 'POST', headers,
        body: JSON.stringify({ metered: mtrB2C(), task: 'tuteur_chat', data: { message, niveau, matieres, cours, historique, internet, prenom, langue } }),
      })
      const json = await res.json().catch(() => null)
      noteCredits(json)
