<template>
  <svg class="radar6c" :viewBox="viewBox" role="img" aria-label="Radar de compétences">
    ...
    <!-- Libellés -->
    <text v-for="(c, i) in comps" :key="'lbl' + i"
...
const props = defineProps({
  ...
  // Sous-ensemble de compétences à afficher (clés). Vide = toutes.
  keys: { type: Array, default: () => [] },
})

// Compétences affichées : le sous-ensemble demandé (ex. les forces), sinon toutes.
const comps = computed(() => {
  if (props.keys && props.keys.length >= 3) return COMPETENCES_6C.filter((c) => props.keys.includes(c.key))
  return COMPETENCES_6C
})
const N = computed(() => comps.value.length)

// Rayon adapté au nombre d'axes ; viewBox large pour les libellés (10 axes).
const cx = 160
const cy = 130
const R = 78
const viewBox = '-40 0 400 280'

function angle(i) { return (-90 + i * (360 / N.value)) * Math.PI / 180 }
function scoreOf(i) {
  const v = Number(props.scores?.[comps.value[i].key] || 0)
...
function ringPoints(ring) {
  return comps.value.map((_, i) => { const p = vertex(i, ring); return `${p.x.toFixed(1)},${p.y.toFixed(1)}` }).join(' ')
}
const scorePoints = computed(() =>
  comps.value.map((_, i) => { const p = point(i); return `${p.x.toFixed(1)},${p.y.toFixed(1)}` }).join(' ')
)
function label(i) {
  const a = angle(i)
  const r = R + 18
...
<style scoped>
.radar6c { width: 100%; max-width: 360px; height: auto; display: block; margin: 0 auto; }
...
.r-label { font-family: 'Poppins', sans-serif; font-size: 9px; font-weight: 600; fill: var(--tx2, #4b5563); }
</style>
