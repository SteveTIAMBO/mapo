<template>
  <div class="crs">
    <div class="crs-head">
      <div>
        <h1 class="crs-h1">{{ t('sup.ensCours.title') }}</h1>
        <p class="crs-sub">{{ t('sup.ensCours.subtitle') }}</p>
      </div>
    </div>

    <!-- Publier -->
    <section class="crs-panel">
      <h2 class="crs-h2">{{ t('sup.ensCours.publish') }}</h2>
      <div class="crs-row">
        <div class="crs-fg">
          <label>{{ t('sup.ensCours.type') }}</label>
          <select v-model="form.type" class="crs-input">
            <option value="cours">{{ t('sup.ensCours.typeCours') }}</option>
            <option value="td">{{ t('sup.ensCours.typeTd') }}</option>
            <option value="ressource">{{ t('sup.ensCours.typeRessource') }}</option>
          </select>
        </div>
        <div class="crs-fg crs-grow">
          <label>{{ t('sup.ensCours.ue') }}</label>
          <select v-model="form.ueId" class="crs-input">
            <option value="">{{ t('sup.ensCours.allUe') }}</option>
            <option v-for="u in mesUe" :key="u.id" :value="u.id">{{ u.code }} · {{ u.intitule }}</option>
          </select>
        </div>
      </div>
      <div class="crs-fg">
        <label>{{ t('sup.ensCours.titleField') }}</label>
        <input v-model="form.titre" class="crs-input" type="text" :placeholder="t('sup.ensCours.titlePlaceholder')" />
      </div>
      <div v-if="form.type === 'ressource'" class="crs-fg">
        <label>{{ t('sup.ensCours.link') }}</label>
        <input v-model="form.url" class="crs-input" type="url" placeholder="https://…" />
      </div>
      <div class="crs-fg">
        <label>{{ t('sup.ensCours.description') }} <span class="crs-opt">{{ t('sup.ensCours.optional') }}</span></label>
        <textarea v-model="form.description" class="crs-input" rows="4" :placeholder="t('sup.ensCours.descPlaceholder')"></textarea>
      </div>
      <div class="crs-actions">
        <button class="crs-btn" type="button" :disabled="!canPublish" @click="publier">
          ...
          {{ t('sup.ensCours.publishBtn') }}
        </button>
        <transition name="crs-fade"><span v-if="justPublished" class="crs-ok">{{ t('sup.ensCours.published') }}</span></transition>
      </div>
    </section>
    ...
    <section class="crs-panel">
      <div class="crs-panel-head">
        <h2 class="crs-h2">{{ t('sup.ensCours.publishedContent') }}</h2>
      ...
      <div v-if="items.length" class="crs-items">
        <div v-for="it in items" :key="it.id" class="crs-item" :class="'t-' + it.type">
          <div class="crs-item-main">
            ...
            <strong class="crs-item-title">{{ it.titre || t('sup.ensCours.untitled') }}</strong>
            ...
            <a v-if="it.url" :href="it.url" target="_blank" rel="noopener" class="crs-link">
              ...
              {{ t('sup.ensCours.openLink') }}
            </a>
            <div class="crs-item-meta">{{ moi.nomComplet }} · {{ fmtDate(it.createdAt) }}</div>
          </div>
          <button class="crs-del" type="button" :title="t('sup.ensCours.delete')" @click="supprimer(it.id)">
      ...
      <p v-else class="crs-empty">{{ t('sup.ensCours.empty') }}</p>
    </section>
  </div>
</template>
...
function typeLabel(ty) {
  return ty === 'cours' ? t('sup.ensCours.typeCours') : ty === 'td' ? t('sup.ensCours.typeTd') : t('sup.ensCours.typeRessourceShort')
}
