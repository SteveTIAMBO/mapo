        ...
        <div class="modal-body">
          ...
          <div v-if="compose.recipientType === 'class'" class="form-group">
            ...
            <select v-model="compose.recipientValue" class="select" style="width: 100%;">
              ...
              <option v-for="p in superieurStore.promotions" :key="p.id" :value="p.programmeNom + ' — ' + p.anneeNom">{{ p.programmeNom }} — {{ p.anneeNom }}</option>
            </select>
          </div>
...
// Contexte « parent » inutilisé au Supérieur (le directeur n'est pas un parent)
...
function buildRecipientList() {
  const list = []
  // Intervenants
  superieurStore.intervenants.forEach(it => {
    list.push({
      id: it.id,
      name: `${it.prenom} ${it.nom}`,
      roleLabel: it.statut === 'vacataire' ? 'Intervenant (vacataire)' : 'Intervenant',
      type: 'intervenant',
      role: 'enseignant',
    })
  })
  // Étudiants
  superieurStore.etudiants.forEach(e => {
    list.push({
      id: e.id,
      name: e.nomComplet,
      roleLabel: `Étudiant${e.programmeNom ? ' · ' + e.programmeNom : ''}`,
      type: 'etudiant',
      role: 'etudiant',
    })
  })
